...
// 관계 칩 — 짧은 라벨 + 대표 이모지. 순서는 내 관계를 맨 앞으로(아래 chipOrder).
...
export default function SituationSelect({ status, onPick, onClose }: Props) {
  ...
  // 선택 관계의 상황 2개 + 직접쓰기(custom은 relationship:"any"라 모든 관계 목록에 포함 = "모든 탭에 직접 입력").
            ...
            {situations.map((s) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left rounded-2xl p-4 bg-cream-warm border transition-colors flex items-start gap-3 ${
                    isCustom
                      ? "border-dashed border-lilac-mid/50 hover:border-lilac-deep/50"
                      : "border-lilac-mid/20 hover:border-lilac-deep/40"
                  }`}
                >
      ...
      <div className="bg-cream rounded-t-3xl min-h-[70dvh] p-4">
        ...
        <div className="mt-4 flex flex-col gap-2">
          <button
            ...
            className="w-full rounded-xl py-3 bg-lilac-deep text-white font-bold disabled:opacity-40 disabled:cursor-not-allowed"
          >
            시작하기
          </button>
          {!isCustom && (
            <button onClick={() => onPick(picked.id, "")} className="w-full rounded-xl py-2.5 text-text-light text-sm">
              건너뛰고 바로 시작
            </button>
          )}
          {isCustom && !ctx.trim() && (
            <p className="text-xs text-text-light text-center">직접 쓰기는 상황을 한 줄이라도 적어줘</p>
          )}
        </div>
      </div>
    </section>
