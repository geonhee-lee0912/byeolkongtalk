...
| 4 | 5별 연장 턴수 | **+5턴** (반복 가능, 보수적 — 과금 분산·마찰 유지) |
...
## 2. 상대 등록 & 온보딩 (`/relationship`)

- **미등록**: 콜드스타트 = 마케팅 지면 (스펙 §5). "우리 사이" 소개(지속 대화형 연애 상담 + 스킬) + 상대 등록 CTA.
- **등록 온보딩**: 호칭 → 관계 상태(4택) → 양쪽 생년월일시(내 것 = primary 프로필 재사용/신규, 상대 = partner 프로필). 생일은 "궁합 스킬에 필요, 나중에 추가 가능"으로 선택적 진행 허용(단 궁합 스킬은 미등록 시 잠금).
- 등록 완료 → `relationships` INSERT + 스레드 본체 reading INSERT(`thread_reading_id` 설정) → 스레드 진입.
- **등록됨 + 패스 없음**: 스레드 히스토리 열람 가능(읽기 전용) + "패스 구매하고 이어가기".
- 로그인 가드: 미인증 → `/login?next=/relationship`.
...
2. **일일 소프트캡**: "오늘"=KST 캘린더일. 오늘 스레드 user 턴 수 계산(messages.created_at, KST). 허용량 = `20 + 5 × (오늘 연장 횟수)`.
   ...
   - **연장(결정 4)**: 5별 → `spend_stars(source='rel_extend')` → 그 즉시 오늘 허용량 +5. 오늘 연장 횟수 = 오늘(KST) `star_transactions where source='rel_extend'` count (별도 카운터 테이블 불필요).
...
## 4. 스킬 4종 (결정 6: 마커 칩 + 접힌 상시 메뉴)

인스레드 상품. **활성 패스 필요**(스펙 §6 "패스 선구매 후의 인챗 상품"). 스킬 = `relationship_id` 부착된 자식 reading. 완료 시 결과 요약을 스레드 노트 + `memo.skill_log`에 반영 → 별콩이 이후 기억.

| 스킬 | 가격 | 구현 = 재사용 |
|---|---|---|
| 관계 체크인 | 45 | `checkin_6` 스프레드 draw → 타로 reading |
| 걔 속마음 | 40 | `deep_feelings_5` 스프레드 draw → 타로 reading |
| 우리 궁합 | 40 | 등록된 두 프로필 → compat 리포트 |
| 싸움 잘잘못 판정 | 30 | **신규 대화형**(드로우 없음): 양쪽 입장 청취 → 비율 판정 → 화해 처방 |

- **실행 UX**: (a) 별콩이가 맥락상 적절할 때 마커(예: `[SKILL:checkin]`) → 칩 노출, (b) 스레드에 접힌 상시 스킬 메뉴(4종 + 가격) 상시 접근.
- 스프레드 2종(체크인/속마음)은 기존 draw 흐름 재사용(별 차감·draw·SSE). 궁합은 compat 파이프라인. 판정은 신규 프롬프트 흐름.
...
- **2a 기반**: 마이그레이션(relationships + passes + readings ALTER) · 상대 등록 온보딩 · 우리 사이 탭 · 콜드스타트 페이지 · 보관함→내정보 이동.
- **2b 스레드 + 패스**: 관계 채팅 라우트(무수렴·패스 게이트·소프트캡·5별 연장) · 패스 구매 RPC/UI(한도 명시) · 기억(파일+최근N+임계요약) · 관계 에이전트 페르소나 블록.
- **2c 스킬 4종**: 마커+칩 + 접힌 메뉴 · 4스킬(체크인·속마음·궁합·판정) · 결과 스레드/`memo` 반영.
...
1. 미등록 `/relationship` = 콜드스타트, 등록 온보딩(호칭·상태·양쪽 생일) → relationships + 스레드 생성.
2. 패스 없이 전송 → 402 → 패스 구매(1일20/3일40/7일60, 한도 명시) → 스레드 대화 가능.
3. 패스 활성 중 재구매 → 만료가 이어붙음.
4. 하루 20턴 도달 → 부드러운 마무리(스레드 유지) + 5별 연장 → +5턴 재개.
5. 스레드 깊어져도 기억 유지(최근 원문 + 요약 + 파일), 응답 지연 없음(요약 백그라운드).
6. 스킬 4종: 마커 칩 + 접힌 메뉴 양쪽에서 실행, 별 차감, 결과가 스레드에 기억됨.
7. 처방 후 재방문 시 별콩이가 먼저 안부(체크인) → 소진.
8. 탭 "우리 사이" 노출, 보관함은 내 정보에서 접근, 스레드 본체는 보관함 미노출.
9. 회원 탈퇴 시 relationships/passes/스레드/스킬 reading CASCADE 정리.
