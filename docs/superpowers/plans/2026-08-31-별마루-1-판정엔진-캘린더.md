...
**이 계획의 범위 밖 (별도 계획):** 구독 기간권·블러·3일 체험(②) / 우리 오늘(③) / 계측·탭 이름 재편·`good_days`·`one_card` 폐지(④) / 정적 콘텐츠 생성·검수(⑤) / 일지 캐릭터 에셋(⑥)
...
| `components/layout/BottomTab.tsx` (수정) | 별마루 탭 추가 |
...
이 계획에서는 **추가만** 한다. `우리 사이` 제거와 탭 이름 재편은 ④ 계획(계측·탭 재편)에서 원자적으로 처리한다 — 지금 제거하면 `/relationship` 진입 경로가 사라지고 별마루는 아직 대체품이 아니다.

- [ ] **Step 1: `TabKey` 에 키를 추가한다**

`components/layout/BottomTab.tsx:7` 을 수정:

```tsx
type TabKey = "consult" | "fortune" | "history" | "byeolmaru" | "shop" | "me";
```

- [ ] **Step 2: 탭 항목을 추가한다**

`TABS` 배열에서 `우리 사이`(`key: "history"`) 항목 **바로 뒤**에 추가한다. 아이콘은 기존 항목과 같은 규약 — 24×24 좌표계 · 솔리드 글리프 · 별 구멍은 `iconEvenOdd: true`:
...
기대: 오류 0. 빌드 라우트 표에 `/byeolmaru` 와 `/api/byeolmaru/calendar` 가 뜨고, `ƒ Proxy` 줄이 그대로 있는지 확인한다(proxy 인식이 깨지면 anon 쿠키 미발급 + `/admin` 가드 소실 — AGENTS.md).

- [ ] **Step 4: 브라우저로 확인한다**

`navigate` `http://localhost:3000/` → `read_page` → 하단탭에 `별마루` 가 보이고, `find({ query: "별마루" })` 로 얻은 ref 를 클릭하면 `/byeolmaru` 로 이동하는지 확인.

기대: 탭 6개가 한 줄에 들어가되 레이아웃이 깨지지 않는다. **깨지면 이 태스크를 멈추고 보고한다** — 탭 6개는 임시 상태이고, ④ 계획에서 `우리 사이` 제거로 5개가 된다. 임시로 깨지는 게 확인되면 ④를 먼저 할지 판단이 필요하다.

- [ ] **Step 5: 커밋**

```bash
git add components/layout/BottomTab.tsx
git commit -m "feat(byeolmaru): 하단탭에 별마루 추가"
```
...
② 구독 기간권 + 블러 게이트 + 3일 체험 → ③ 우리 오늘 → ④ 계측 + 탭 재편 + `good_days`/`one_card` 폐지 → ⑤ 정적 콘텐츠 1회 생성·검수 → ⑥ 일지 캐릭터 에셋
