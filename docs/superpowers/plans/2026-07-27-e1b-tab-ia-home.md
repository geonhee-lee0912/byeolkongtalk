...
| 캐러셀 | 비로그인 미노출 · 클릭 시 `ui_events.banner_clicked{slot}` · reduced-motion 존중 |
...
- 배너 클릭 계측 작동 확인 — `ui_events` 에서 `banner_clicked{slot}` 행이 쌓이는지. ⚠️ (2026-08-14 정정) 원안의 `?b=`(page_views)는 **미도달이었다** — `page_views` 는 쿼리스트링을 저장하지 않고 `normalizePath` 가 잘라 `?b=` 가 URL 에만 존재했다(리포 전체 소비처 0). `ui_events.banner_clicked` 로 교체됨
...
- [ ] 캐러셀 최대 3장 + 1번 카드 우선순위 + 도트 + `banner_clicked` 계측(ui_events) — Task 8
