...
CREATE TABLE IF NOT EXISTS llm_usage (
  ...
  cost_won INT,                                          -- NULL = 단가 미확정 (0 과 구별된다)
...
import { costWon, RATES, type Usage } from "@/lib/claude/pricing";
...
export async function recordUsage(
  ...
  if (cost === null && RATES[model] === undefined) {
    // 등록조차 안 된 모델 = pricing.ts 에 줄을 안 넣었다는 뜻. 조용히 새면 안 된다.
    await logWarn("llm 단가 미등록 모델 — cost_won 이 NULL 로 적재된다", {
      ...ctx,
      extra: { ...ctx?.extra, model },
    });
  }
