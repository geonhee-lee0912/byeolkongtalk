...
🎉 **Task 1~9 전부 완료 (2026-09-20).** 게이트: `tsc` 0 · 유닛 **736/736** · `build` 0.
`origin/dev` 푸시 완료 · Supabase **dev 에 마이그레이션 적용 확인** · 카나리아 통과.
🔴 **prod(main) 미배포** — 사용자 명시 go 전까지.

| Task | 커밋 | 비고 |
|---|---|---|
| 1 지표 레지스트리 | `c6fb8fd` | 리뷰가 키 타입 안전성 지적 → `as const satisfies` 로 유니온화 |
| 2 pricing.ts | `fd17153` | 리뷰가 Critical 1 잡음 — `providerOf` 가 haiku 를 몰라 적재가 100% 죽을 뻔 |
| 3 llm_usage 마이그레이션 | `83eff0d` | 안 쓰이는 인덱스 제거 + `provider` CHECK |
| 4~6 어댑터 3종 | `dff5b69` | 통합 리뷰가 회계 버그 2건 — openai 캐시쓰기 미차감(주력 경로 ~10% 과소)·gemini thoughts 누락 |
| 7 usage-log + streamChat | `9e17052` | `recordUsage` 는 **재시도 루프 안**에 있어야 한다 |
| 8 우회 2곳 | `cc96460` | 착점 3/3 |
| 최종 리뷰 수정 | `4cf135f`·`d0624e9` | 경고 dedup 일반화 · UV/탈퇴율 정본 추가 · 민감 판정 `userId` 관통 |

### 카나리아 결과 (Task 9, dev 실호출)

| 확인 | 결과 |
|---|---|
| Supabase dev 적용 | `llm_usage` 존재 · `rls=true` · ACL `{postgres, service_role}` (anon·authenticated **없음**) · 인덱스 3 |
| **anon 실물 HTTP** | **401 / code 42501** "permission denied". 대조군(없는 테이블)은 404 — AGENTS.md 가 경고한 "404 니까 막혔다" 오판을 배제 |
| openai 경로 | `openai/gpt-5.6-luna` in=42 out=13 **cost=0.0336** |
| anthropic 우회 경로 | `anthropic/claude-haiku-4-5-20251001` in=358 out=89 **cost=0.8994** |

🔴 **`NUMERIC` 결정이 첫 실호출에서 입증됐다** — `0.0336`·`0.8994` 는 원안(`INT` + `Math.round`)이었다면
**둘 다 0** 이다. 산수도 정확: `(42×0.2 + 13×1.2)/1e6 × 1400 = 0.0336` · `(358×0.8 + 89×4)/1e6 × 1400 = 0.89936`.
`provider=anthropic` 이 나온 것은 **`providerOf` 수정의 검증**이다(고치기 전이면 throw 로 행이 아예 안 생긴다).

⚠️ **검증 안 된 것 1건** — 카나리아의 시스템 프롬프트가 20자라 캐시 최소 길이에 못 미쳐 `tokens_cache_read`
가 0 이다. **캐시 마킹이 실제로 먹는지는 이 카나리아로 확인되지 않았다.** 실트래픽에서
`tokens_cache_read` 가 계속 0 이면 입력비 ~35% 가 걸린 사안이므로 조사할 것.

**최종 리뷰 판정**: Critical 0. 프로덕션 LLM 호출 15개 지점이 전부 `recordUsage` 에 도달하고,
**실측 누락률 0.01%**(일시적 에러 재시도 1건 / assistant 메시지 9,953건, 전 기간).

🔴 **플랜 B 가 이어받을 것**: 소표본 게이트 React 컴포넌트 · `result_viewed` 실구현 교체(현재
리딩 기준 50.7% ↔ 정본 코호트 기준) · `NUMERIC` 은 PostgREST 가 **문자열**로 주므로
`SUM(cost_won)` 소비처에 `Number()` 캐스트 필요.
...
- [x] **Step 2: 본문을 prod 에서 먼저 돌려본다 (읽기 전용 검증)**
...
- Modify: `lib/claude.ts:540` — **한 줄만** (아래 Step 2-b)
...
- [x] **Step 2: anthropic 어댑터가 usage 를 모은다**
...
- [x] **Step 2-b: `streamChat` 이 새 반환 모양을 읽게 한다 (한 줄)**

🔴 **이 한 줄이 없으면 Task 4·5·6 이 다 끝나도 tsc 가 빨갛다.** `lib/claude.ts:540` 이 제너레이터 반환값을
`StopReason` 변수에 그대로 대입하는데, 계약이 바뀌는 순간 여기가 먼저 깨진다(2026-09-20 Task 4 구현자가
잡은 플랜 오류 — 원안은 이 파일을 Task 7 로 미뤄 세 태스크의 게이트를 전부 무의미하게 만들었다).

`lib/claude.ts:540` 현재:
```ts
      stopReason = r.value;
```
이것으로:
...
⚠️ `lib/claude.ts` 에 이 한 줄 외에 아무것도 건드리지 않는다. `recordUsage` 호출은 Task 7 소유고,
`summarizeOlder`(904행 근처)는 Task 8 소유다. 지금 `usage` 는 버려지는 게 맞다.

- [x] **Step 3: 타입 게이트**

Run: `npx tsc --noEmit`
Expected: **FAIL, 단 `openai.ts` 와 `gemini.ts` 두 파일에만.** 둘이 아직 `StopReason` 을 반환해 계약과 안 맞는다(Task 5·6 에서 고친다).

에러 목록에 `types.ts` · `anthropic.ts` · `lib/claude.ts` 가 있으면 **여기서 멈추고 고친다.** 그 밖의 파일이
나오면 예상 못 한 파급이니 **멈추고 보고한다.**

Run: `node --import tsx --test "lib/**/*.test.ts"`
Expected: 기존 전량 PASS (계약 변경이 유닛 테스트를 깨지 않아야 한다)

- [x] **Step 4: 커밋**

```bash
git add lib/claude/adapters/types.ts lib/claude/adapters/anthropic.ts lib/claude.ts
...
> 🔴 **2026-09-20 통합 리뷰 정정 (커밋 `dff5b69`) — 아래 Step 2 코드는 낡았다.**
> `cacheWriteTokens: 0` 과 "OpenAI 는 캐시 쓰기를 따로 청구하지 않는다" 주석이 **틀렸다.**
> `gpt-5.6-luna` 는 캐시 쓰기를 $0.25/MTok 로 **실제 청구**하고, `prompt_tokens` 는
> `ordinary + cached + cache_write` 라 **둘 다 빼야** 한다. 안 그러면 캐시쓰기분이 입력 요율로
> 계상돼 참 비용의 20% 가 증발한다(혼합 기준 luna chat 원가의 ~10% 과소, 하필 `CHAT_MODEL`).
> 현재 코드는 `lib/claude/adapters/openai.ts` 의 `mapOpenAIUsage` 가 정본이고, 계약은
> `lib/claude/adapters/usage-mapping.test.ts` 가 지킨다.

**Files:**
- Modify: `lib/claude/adapters/openai.ts:60-82`

- [x] **Step 1: `stream_options` 를 켠다**
...
> 🔴 **2026-09-20 통합 리뷰 정정 (커밋 `dff5b69`) — 아래 Step 1 코드는 낡았다.**
> `candidatesTokenCount` 에 **thoughts 가 안 들어간다**(`genai.d.ts` 의 `totalTokenCount` 주석).
> 이 어댑터는 thinking 을 켜므로(3.x 는 못 끔) thoughts > 0 이 보장되고 구글은 출력 요율로 청구한다.
> `outputTokens` 는 `candidatesTokenCount + thoughtsTokenCount` 다. 현재 정본은
> `lib/claude/adapters/gemini.ts` 의 `mapGeminiUsage`.

**Files:**
- Modify: `lib/claude/adapters/gemini.ts:45-52`

- [x] **Step 1: `usageMetadata` 를 읽는다**
...
- [x] **Step 2: `streamChat` 이 usage 를 벗겨 적재한다**
...
```ts
      ...
      stopReason = r.value.stop;   // ← Task 4 Step 2-b 에서 이미 이렇게 돼 있다
      // 시도가 끝까지 갔으면 토큰은 실제로 소비됐다 → 재시도로 버려질 시도도 기록한다.
      void recordUsage(resolved, r.value.usage, logCtx);   // ← Task 7 이 추가하는 줄은 이것 하나다
```

> 🔴 **`recordUsage` 는 반드시 재시도 루프 *안*(540행 자리)에 둔다.** 밖으로 빼면 빈 완료 재시도로
> 버려진 시도의 입력 토큰이 통째로 증발한다 — `streamChat` 은 0자 완료 시 재호출하는데, 그 시도도
> 입력 토큰은 이미 청구된 상태다. 루프 안에 두면 시도별로 각각 적재된다(2026-09-20 통합 리뷰 지적).
>
> ⚠️ **알려진 한계 2건** (플랜이 받아들이는 손실):
> 1. **throw 된 시도의 토큰은 기록되지 않는다** — 제너레이터가 return 값을 못 준다.
>    `yielded===true` 상태의 throw(`lib/claude.ts:558`)와 클라이언트 abort 가 여기 해당하고,
>    그때 입력 토큰 전량 + 부분 출력은 이미 청구된 뒤다. **손실 1건 = 최대 한 턴 원가(luna ₩2~12)**
>    이므로 에러율이 곧 편향률이다 — `/admin/errors` 의 `streamChat` 건수 ÷ 총 턴수로 측정할 수 있고,
>    1% 미만이면 한계로 두는 판단이 유효하다.
> 2. **어댑터의 `Math.max(0, ...)` 가 계약 위반을 조용히 삼킨다** — `cached > prompt_tokens` 같은
>    이상 응답이 오면 `inputTokens: 0` + 부풀려진 `cacheReadTokens` 가 신호 없이 적재된다.
>    클램프가 어댑터 안에 있어 `recordUsage` 는 음수를 볼 수 없으므로 여기서 감지할 방법이 없다.
>    관측되면 그때 어댑터에 신호를 추가한다(지금 추측으로 만들지 않는다).
...
- [x] **Step 2: dev DB 에 마이그레이션을 적용한다**
