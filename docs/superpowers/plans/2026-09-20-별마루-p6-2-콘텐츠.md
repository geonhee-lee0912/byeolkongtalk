...
🔴 **실행 중 확정한 구조 변경 2건 (2026-09-20, 아래 태스크 본문보다 이게 우선한다)**

이 플랜은 원래 Task 5 와 Task 6 에서 **tsc 가 일시적으로 깨지는 것을 감수**하도록 썼다(구 심볼을 먼저 지우고 Task 8 이 라우트를 갈아끼우는 순서). 실행에서 이를 뒤집었다 — 리뷰어가 매 커밋 `tsc` 를 돌려 판정하는데, **의도적으로 깨진 게이트는 리뷰 결과를 못 읽게 만든다.**

1. **Task 5 는 구 심볼을 지우지 않는다.** `buildCardNarrativeSystem` · `CARD_NARRATIVE_KICKOFF` · `CARD_NARRATIVE_MAX_TOKENS` 를 `@deprecated` JSDoc 만 붙여 그대로 두고, 신규 API 를 **추가**만 한다. 제거는 마지막 호출부(라우트)를 없애는 커밋이 함께 한다.
2. **Task 6 과 Task 8 은 한 커밋으로 합친다.** 저장 포맷(JSONB) 교체와 그 생산자·소비자 교체는 원래 원자적 변경이다. 따로 커밋하면 중간 상태에서 래퍼의 새 export 와 라우트의 옛 import 가 어긋나 tsc 가 깨진다.

**순서 변경도 1건:** Task 7(형제 래퍼 계측·pair 승자 응답)은 5·6·8 과 파일이 겹치지 않고 tsc 를 깨지 않아 **Task 5 보다 먼저** 실행했다. 의존성 없음.
