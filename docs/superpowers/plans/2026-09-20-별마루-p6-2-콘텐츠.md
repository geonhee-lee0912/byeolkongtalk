...
**커밋 규율:** `git add <파일 지정>` 만. `git add .`/`-A` 금지(작업 트리에 미커밋 findings 문서 3개 + `scripts/roadmap-kpi-snapshot.sql` 이 있다). 메시지 끝에 `Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>`.
