...
> ```ts
> /**
>  * 창 **끝에서부터** 광고비가 0 인 연속 일수.
>  *
>  * ⚠️ "광고를 안 쓴 날"과 "아직 입력을 안 한 날"은 **구분되지 않는다** — ad_spend 에는
>  *    llm_usage 의 cost_rows 같은 존재 신호가 없다. 그래서 이 값은 **상한**이다.
>  *    그래도 보여주는 쪽이 맞다: 안 보여주면 기여가 조용히 과대로 읽힌다.
>  */
> export function adSpendStaleDays(days: DailyPnl[]): number {
>   let n = 0;
>   for (let i = days.length - 1; i >= 0 && days[i].adSpendWon === 0; i--) n++;
>   return n;
> }
> ```
>
...
>
> ```ts
> test("adSpendStaleDays — 창 끝의 빈 날만 센다 (중간 공백은 안 센다)", () => {
>   const d = (adSpendWon: number): DailyPnl => day("2026-09-20", 0, adSpendWon, 0, 0);
>   assert.equal(adSpendStaleDays([d(100), d(0), d(100), d(0), d(0)]), 2);
>   assert.equal(adSpendStaleDays([d(100), d(100)]), 0);
>   assert.equal(adSpendStaleDays([d(0), d(0)]), 2); // 전부 비면 전부 센다
