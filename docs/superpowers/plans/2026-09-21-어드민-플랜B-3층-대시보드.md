...
7. 커밋 메시지는 리포 관례(`feat(admin): …` · `fix(admin): …`)를 따른다. **`Co-Authored-By` 줄은 이 플랜이 지정하지 않는다** — 커밋을 만드는 세션·에이전트가 자기 규칙을 따른다(리포 히스토리에 이미 여러 모델이 혼재하고, 그게 정상이다). 아래 커밋 블록들에 그 줄이 없는 것은 누락이 아니다.
