...
1. 🔴 **`git add -A` / `git add .` 금지 — 그리고 그것만으로는 부족하다.** 별마루 P6 세션이 같은 `dev` 워킹 트리에서 동시에 일한다. 각 Step 의 `git add` 는 경로를 명시했으니 그대로 쓰되, **이미 인덱스에 올라와 있는 남의 변경은 같이 커밋된다.**
   - 2026-09-24 에 실제로 물렸다: 플랜 문서 하나만 `git add` 했는데 별마루가 스테이징해둔 삭제 2건(`components/byeolmaru/PremiumBlock.tsx`·`useBaitDismiss.ts`)이 그 커밋에 딸려 들어갔다.
   - **방어 둘 중 하나를 반드시 쓴다**: 커밋 직전 `git diff --cached --name-only` 로 **스테이징 전체를 눈으로 확인**하거나, `git commit -m "…" -- <경로>` 처럼 **pathspec 으로 커밋 범위를 못박는다**(인덱스의 다른 변경을 건드리지 않는다).
   - 이미 딸려 들어갔다면 `git reset --soft HEAD~1` 로 커밋만 취소하면 인덱스가 보존되므로, 그 상태에서 pathspec 커밋으로 다시 만들면 남의 스테이징이 원래대로 남는다.
...
7. 🔴 **동적 색은 Tailwind 클래스가 아니라 `style={{}}` 로 준다.** (Task 6 에서 실측 확인)
   `` className={`text-[${STATUS.serious}]`} `` 처럼 **보간한 arbitrary value 는 Tailwind 정적 스캔에 안 걸려 CSS 규칙이 아예 생성되지 않는다** — 컴파일도 빌드도 테스트도 전부 통과하는데 **화면에서 색만 조용히 사라진다.** 경보 색이 안 뜨는 건 "경보가 없다"로 읽히므로 이 클래스의 결함은 치명적이다.
   - 리포 선례가 그렇다: `components/saju/SajuBoard.tsx`(`ELEMENT_COLORS`)·`components/admin/CohortHeatmap.tsx` 가 전부 `style={{}}` 로 동적 색을 준다. 보간 클래스 선례는 **0건**(grep 확인).
   - 알파는 헥스 서픽스로: `bg-[#d03b3b]/10` → `${STATUS.critical}1a`(0x1a=26, ≈10.2%) · `/15` → `26`(0x26=38, ≈14.9%).
   - 아래 코드 블록들에 남아 있는 완성 리터럴(`text-[#ec835a]` 등)은 **그대로 두면 동작하지만**, 상수로 바꿀 때는 반드시 `style={{}}` 로 옮겨라.
8. 커밋 메시지는 리포 관례(`feat(admin): …` · `fix(admin): …`)를 따른다. **`Co-Authored-By` 줄은 이 플랜이 지정하지 않는다** — 커밋을 만드는 세션·에이전트가 자기 규칙을 따른다(리포 히스토리에 이미 여러 모델이 혼재하고, 그게 정상이다). 아래 커밋 블록들에 그 줄이 없는 것은 누락이 아니다.
...
| `lib/admin/colors.ts` | 어드민 시각 어휘(상태색 4 · 골드 · 카테고리컬 8) — 스펙 §8 이 정본 | 신규 |
...
> 🔴 **구현 후 리뷰로 추가된 것 3가지** (위 코드 블록에는 없다 — 실제 파일이 정본):
> - **`Band.flat`** — 8주 롤링 값이 **전부** 같은가(`sorted[0] === sorted[at(-1)]`). 없으면 화면이 "평소 범위 안 · 분포 중 100% 위치"를 그려 **무변동을 역대 최고로** 읽게 만든다.
>   ⚠️ **`p10 === p90` 으로 정의하면 안 된다.** 그건 중간 80% 만 같아도 참이라, 꼬리(= 오늘일 수 있다)가 진짜 이상치인 경우 `flat && outside` 가 동시에 참이 된다 — 재리뷰가 `[50, ...×54 100, 200]` 로 실행 재현했고, 그러면 `BandGauge` 의 flat 우선 분기가 **진짜 경고를 삼킨다.**
> - **표본 하한 8의 경계 테스트** — 8개 통과 / 7개 null. `< 8` 이 `<= 8` 로 바뀌는 회귀를 잡는다.
> - **꼬리 이상치 테스트** — `p10 === p90` 인데 `flat: false` · `outside: true` 임을 잠근다.
...
> 🔴 **구현 후 리뷰로 바뀐 것 2가지** (위 코드 블록에는 없다 — 실제 파일이 정본):
> - **게이트는 `den` 을 그대로 쓴다** — `sampleGate(key, n)`. 구 코드의 `unit === "count" ? 0 : n` 분기는 **오늘 동작에 기여하는 게 없으면서**(리뷰어가 제거 후 9/9 통과로 확인) den 을 버려, 훗날 표본 게이트가 필요한 카운트 지표가 생기면 `0 >= minSample` 이 거짓이라 **영원히 "판단 보류"** 로 가두는 경로였다. 크래시도 로그도 없다.
> - **값 선택은 `percent` 기준** — `m.unit === "percent" ? pct1(num, den) : row.num`. 구 코드("count 가 아니면 전부 비율")는 `GUARDRAILS` 에 `won`·`ratio` 지표가 들어오는 순간 **100배 어긋났다.** 그 가정은 `lib/admin-metrics.test.ts` 의 계약 테스트("가드레일 6종은 percent 또는 count 여야 한다")가 지킨다.
...
두 본문을 리터럴 치환해 넣고 실행한다. 창은 **`timestamptz '2026-08-22 00:00+09'` ~ `'2026-09-20 00:00+09'`**, `p_exclude` 는 `left(id::text,8) NOT IN (...)` 형태로 바꾼다.

Run: `SUPABASE_PAT=... node scripts/run-prod-query.mjs <파일>`

> 🔴 **창을 08-21 이 아니라 08-22 부터로 잡는 이유** (Task 5 에서 실측으로 확정):
> 스펙 §2-4 의 "최근 30일" 수치는 **정각 창이 아니라 09-20 당시의 애드혹 스냅샷**(`now() - 30 days` 류)이라 정각으로는 재현되지 않는다. 확인한 근거 3가지:
> - `ad_spend` 의 08-21 하루치가 **정확히 ₩7,802** 이고, 그게 08-21 포함 시 생기던 차이와 **한 원도 안 틀리고 일치**한다
> - **`08-22 00:00+09` ~ `09-20 00:00+09` 창의 `ad_spend` 합 = ₩328,042** — §2-4 값과 **완전 일치**
> - 같은 창의 가입은 **894명**이다. §2-4 의 914 는 여기에 **09-20 부분일**(그 날 가입 35명 중 20명쯤)이 더해진 값 — `ad_spend` 는 `spend_date` **날짜** 비교라 09-20 이 안 들어가고, `users` 는 **시각** 비교라 당일분이 섞인 것이다
>
> 지인 6명은 이 창에 가입이 **0명**이라 제외 로직은 숫자에 영향을 주지 않는다(따라서 signups 차이는 제외 버그가 아니다).

**합격 조건:**
| 값 | 기대 | 판정 |
|---|---|---|
| `unit.ad_spend_won` | **328,042** | 🔴 **정확히 일치해야 한다** — 이 창에서 재현되는 고정값이다 |
| `unit.signups` | **894** | 🔴 정확히 일치해야 한다 |
| `guard.login_success_rate` | ≈ **67~68%** | §2-6 이 67.6%(1,035/1,531). 창이 조금 달라도 비율은 이 근처 |
| `guard.first_reading_rate` | **80% 이상** | §3 표의 88.2% 는 또 다른 창의 값이다. 경보선(<80%)을 넘지 않는지만 본다 |
| `guard.result_viewed` | **60% 이상** | 🔴 **50%대면 정의가 리딩 기준으로 샌 것이다**(리딩 기준은 50.7%). 정본은 코호트 기준 |

🔴 **`result_viewed` 가 50%대면 멈춘다** — `first_reading` CTE 가 `DISTINCT ON` 으로 유저당 1행인지, `stars_spent > 0` 필터가 **분자·분모 양쪽**에 걸렸는지 확인한다. 이 정의가 1층 가드레일과 3층 판정을 같은 말로 묶는 지점이다(스펙 §7).

⚠️ **`unit.revenue_won` 에는 기대값을 두지 않는다** — 코호트 귀속이라 "그 창에 가입한 사람이 **전 기간** 낸 매출"이고, 조회 시점이 늦을수록 커진다. 자릿수만 본다.
...
> ✅ **완료 (`2d82082`). 실제 파일이 정본이다** — 아래 코드 블록에는 **결함 4건이 남아 있으니 그대로 복사하지 마라.** 스펙 리뷰가 전부 확인했다:
> 1. `GuardrailRow` 의 import 에 **`type MetricDef` 가 빠져 있다**(주석은 "명시 필요"라 써놓고). 그대로 쓰면 tsc 가 죽는다.
> 2. **동적 색이 Tailwind 리터럴 클래스로 남아 있다** — 상수로 바꾸려면 `style={{}}` 로 옮겨야 한다(실행 규율 7). 플랜 자신이 광고비 경고에는 이미 `style={{ color: STATUS.warning }}` 를 쓰면서 나머지는 안 바꾼 **내적 비일관성**이었다.
> 3. **`adStaleDays` 가 Step 4 코드에서 3곳 누락**(import · `loadLayer1` 반환 · JSX prop). `ContributionHero` 가 **필수** prop 으로 선언하므로 안 채우면 tsc 가 죽는다. Step 0-b 서술에는 있었다.
> 4. **Step 6 의 `git add` 목록이 4개뿐**이라 `lib/admin/colors.ts`(없으면 빌드가 깨진다)·`band.ts`·`band.test.ts`·`Metric.tsx` 가 빠진다. 실제로는 8파일이다.
>
> 🔴 **결함 2 는 tsc·유닛·build 가 전부 통과시킨다** — 화면에서 색만 사라진다. 이 플랜에서 자동 게이트가 못 잡은 클래스가 또 하나 나온 것이다.
>
> **실렌더 검증 결과**(dev DB, 임시 어드민 쿠키): 1층 3섹션 정상 · `LoadFailed` 0 · 경보 카드 `#d03b3b`/`#ec835a` 실제 적용 확인 · 광고비 경고 `#fab219` · `band.flat` 분기("변동 없음") 작동 · 소표본 게이트 작동 · 기존 섹션 생존 · 375px 오버플로 0.

**Files:**
- Create: `lib/admin/colors.ts`
- Modify: `lib/admin/band.ts` (+ `band.test.ts`) — `adSpendStaleDays` 추가
...
- Modify: `components/admin/Metric.tsx` (하드코딩 색 → 상수)
- Modify: `app/admin/page.tsx`

> 🔴 **Step 0-b — 광고비 입력 지연을 잴 수 있게 한다** (Task 4 실측에서 드러난 위험).
> `ad_spend` 는 **사용자가 손으로 입력**한다. 2026-09-24 실측에서 마지막 입력이 **09-19** 라 **5일이 비어 있었다** — 7일 창에서 5일이 비면 광고비가 ~71% 과소로 잡히고 **기여가 그만큼 과대**로 보인다. 레지스트리 caveat 에 적어둔 것만으로는 아무도 안 읽는다. 화면이 말해야 한다.
>
> `lib/admin/band.ts` 에 추가(`costCoverage` 옆 — `DailyPnl[]` 를 받는 함수들이 거기 모여 있다):
>
> `DailyPnl` 에 `adRows` 를 추가하고(Task 4 RPC 가 `ad_rows` 를 준다) `costRows` 와 같은 방식으로 판정한다:
>
> ```ts
> export interface DailyPnl {
>   bucket: string;
...
> ```
>
> 테스트도 같이(기존 `day()` 헬퍼에 `adRows` 인자를 끼워 넣어야 하므로 **기존 테스트의 헬퍼 시그니처도 같이 고친다**):
>
> ```ts
...
> ```
>
> `loadLayer1()` 은 `adStaleDays: adSpendStaleDays(last7)` 를 반환하고 `<ContributionHero adStaleDays={L1.adStaleDays} />` 로 넘긴다.

> 🔴 **이 태스크는 기존 섹션(별 소모·연애 상담·처리 대기)을 지우지 않는다.** 1층을 **위에 얹기만** 한다. 2층으로 옮기는 것은 Task 8 이다 — 중간 상태에서도 화면이 온전해야 한다.

> 🔴 **Task 4 실측 — 배포 직후 화면이 "역대급 흑자"로 보일 것이다.** 리뷰어가 prod 데이터로 `lib/admin/band.ts` 실제 함수를 돌려 확인했다:
>
> | 시나리오 | 현재값 | P10~P90 | pctRank |
> |---|---|---|---|
> | 지금 데이터 그대로 | **+58,728원** | −33,883 ~ 11,710 | **100% (P90 의 5.0배)** |
> | 광고비 지연만 제거 | +2,754원 | −33,883 ~ 2,646 | 91% (정상) |
>
> 원인은 코드가 아니라 **광고비 수기 입력 지연**(마지막 입력 09-19, 7일 창에 0원 처리된 날 5/7)이다. 그래서 위 `adSpendStaleDays` 경고가 **없으면 안 된다** — 있으면 사람이 "아, 광고비를 아직 안 넣었구나"로 읽고, 없으면 "장사가 잘 되는구나"로 오독한다.
>
> ⚠️ **원가가 창을 못 덮는 동안(최소 11월 중순까지) 밴드 축은 계속 `marketing`(매출−광고비)** 이다 — `pickBandAxis` 가 그렇게 고른다. 이건 이론이 아니라 향후 두 달간 실제로 작동할 축이다.
>
> 📌 **기록만 하고 지금 고치지 않는 것**: 7일 롤링 창의 마지막 값이 **진행 중인 오늘**을 포함한다(축 끝이 `now()`). 부분일이라 구조적으로 낮게 잡히는 건 맞지만, 실측 효과가 **₩3,032 = 밴드 폭의 5%** 로 작고 지금 관측되는 왜곡을 못 고친다(리뷰어 기각). 광고비 지연이 해소되고 원가가 축적된 뒤에 재검토할 것.

> 🔴 **Task 2 에서 넘어온 인수인계 2건** (코드 리뷰 Minor):
> - **`admin_layer1_pnl` 결과를 가공 없이 그대로 `dailyValues`→`rolling7` 에 넘긴다.** `rolling7` 은 "달력 7일"이 아니라 **배열 7개 원소**를 묶는다. 중간에 필터링하거나 다른 소스와 합치면 창이 조용히 어긋나고 **에러 없이 틀린 값**이 나온다. RPC 가 `generate_series` 로 빈 날도 행을 만들어 주므로 손대지 않으면 안전하다.
> - **밴드 축 전환은 계단식이다.** 원가가 창을 부분적으로 덮다가 전부 덮는 순간 `pickBandAxis` 가 `marketing`→`contribution` 으로 바뀌면서 **밴드 전체가 한 번에 널뛴다**(성분이 달라지므로). 의도한 동작이니 막지 말고, 캡션이 축을 말하게 둔다(아래 `BandGauge` 가 이미 그렇게 한다).

> 🔴 **Step 0 — 상태색을 상수로 먼저 뽑는다** (Task 1 코드 리뷰 지적). 이 태스크가 만드는 컴포넌트 3개 + 이미 있는 `Metric.tsx` + Task 8 의 `DivergingBar` 가 **같은 4색 어휘를 반복**한다. 헥스를 컴포넌트마다 다시 타이핑하면 `formatMetric` 을 만든 이유와 **똑같은 종류의 드리프트**가 색상 쪽에서 재발한다.
>
> ```ts
...
> ```
>
> 그리고 `Metric.tsx` 의 `#d03b3b`·`#ec835a` 리터럴을 `STATUS.critical`·`STATUS.serious` 로 바꾼다. 아래 이 태스크의 컴포넌트 코드들도 같은 상수를 쓴다(코드 블록에는 헥스가 그대로 적혀 있으니 **작성 시 상수로 치환할 것**).
...
> 🔴 색은 `lib/admin/colors.ts`(Task 6 Step 0)에서 가져온다. 아래 코드에 적힌 `#d95926`·`#199e70` 은 카테고리컬 슬롯 2·3 이므로 `CATEGORICAL[1]`·`CATEGORICAL[2]` 로 치환해 쓸 것 — 헥스를 다시 타이핑하지 않는다.
