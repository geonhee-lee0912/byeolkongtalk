...
- [x] P0-1 무료 3턴 (서버 게이트 + 마지막 턴 마무리 가이드 + 클라 개방) — Task 1·2
- [x] P0-2 turn-1 출구 칩 — Task 3
- [x] P1-3 회피 상용구·조건부 범위 답 / P1-4 premium 골격 / P1-5 CARD 마커 — Task 4
- [x] P2-6 reco 중단(인챗 칩·이어가기 보존) — Task 5
- [x] ~~P2-7 죽은 SKU 진열 제거~~ — **제외** (2026-07-26 사용자 결정: 현행 유지)
- [x] P2-8 utm 스냅샷 + landing_variant 분리 — Task 7·8
- [x] prod 배포 + day 0 기준점 확정 — Task 9

## 실행 결과 (2026-07-26)

**prod `48f95bb`** (`852ac05→48f95bb` fast-forward, 20커밋). **이 배포 시각 = 스펙 §5 의 day 0.**
검증: 유닛 179/179 · `npm run build` · dev/prod `/api/health` 200 · prod 공개 라우트 12개 200 · dev Supabase 마이그레이션 적용 실증(신규 8컬럼 select 200) · QA 하네스 타겟 3케이스(관계 무료턴 경계 통과 — 프리시드 3턴 후 4턴째 402, 별 소모 0).

리뷰에서 잡혀 계획 밖으로 추가된 것:
- 무료 인트로 배너가 마운트 시점에 얼어붙어 3턴 내내 "1/3턴" 이던 문제 → `onUserTurnComplete` 콜백으로 실시간화
- 무료 구간에서 스킬 칩을 누르면 별이 충분해도 `/shop` 으로 튕기던 문제 → 402 분기(`INSUFFICIENT_STARS` vs 패스 없음) 구분
- 소진 상태 CTA 가 "패스 연장하기"(한 번도 산 적 없는 유저에게) → "패스 시작하기" + 이 변경이 만든 고아 분기 제거
- 감정별 동적 블록(`lib/emotion-persona.ts`)이 코어보다 **뒤에** 주입되며 P1-3 을 정면으로 취소하던 문제 → 두 태그 재작성
- `PROMPT_VERSION` 미갱신(측정 불가) · QA judge 루브릭이 **금지 대상 문구를 명시 허용**하던 문제 · 하네스 시드 덱이 1번 자리를 항상 "바보"(일반어)로 뽑아 마커 선행 검사를 영영 스킵하던 문제
- `tarot-fresh` 이어가기가 정가(최대 55별) 받고 400~700자 가이드를 쓰던 기존 결함

미해결로 남긴 것:
- **압박 시 과잉 확언**("딱 2주야") — 가드레일 2회 시도가 모두 역효과(회피구 복귀 → 마커 규칙까지 붕괴)라 되돌림. day 4 관찰 항목.
- 7장 첫 풀이 2,960자(하한 3,300 미달) — 과거 1,962자(55%) 대비 90% 로 개선, 회차 변동 있음.
- 사주 `fresh` 이어가기에 동일 결함 잔존 (사주 신규 진입이 닫혀 있어 물량 적다고 판단, 별도 작업으로 분리).
