...
## Task 4b: 대화 길이 — 프리미엄 convergeStartTurn +1 (`lib/tarot/constants.ts`)

**Files:**
- Modify: `lib/tarot/constants.ts`

프리미엄(3장+) 8개 스프레드의 `convergeStartTurn`만 +1. hardCapTurn·hardCapChars·convergeStartChars·absTurnCap·원투카드는 불변. 각 스프레드는 개별 객체라 값 그대로 수정한다.

- [ ] **Step 1: three_card `convergeStartTurn` 5→6**

`three_card` 블록에서 `convergeStartTurn: 5,` → `convergeStartTurn: 6,`

- [ ] **Step 2: 5장 4종 `convergeStartTurn` 7→8**

`relationship_5`, `deep_feelings_5`, `reunion_5`, `new_love_5` 각 블록에서 `convergeStartTurn: 7,` → `convergeStartTurn: 8,` (4곳).

- [ ] **Step 3: 6장 4종 `convergeStartTurn` 8→9**

`checkin_6`, `stay_or_go_6`, `readiness_6`, `healing_6` 각 블록에서 `convergeStartTurn: 8,` → `convergeStartTurn: 9,` (4곳).

- [ ] **Step 4: 7장 3종 `convergeStartTurn` 9→10**

`reunion_deep_7`, `potential_7`, `chakra_7` 각 블록에서 `convergeStartTurn: 9,` → `convergeStartTurn: 10,` (3곳).

- [ ] **Step 5: 정합 확인 — convergeStartTurn < hardCapTurn 유지**

Run: `grep -n "convergeStartTurn\|hardCapTurn" lib/tarot/constants.ts`
Expected: 원카드 3/5, 투카드 4/6, 쓰리카드 **6**/7, 5장 **8**/9, 6장 **9**/10, 7장 **10**/11 — 모든 스프레드에서 convergeStartTurn < hardCapTurn(간격 1 이상).

- [ ] **Step 6: 타입체크**

Run: `npx tsc --noEmit 2>&1 | grep -v verify-offer`
Expected: 에러 0.
...
git commit -m "feat(tarot): 프리미엄 수렴 시작 +1턴 — 자유 대화 구간 확장 (하드캡 불변)"
```
