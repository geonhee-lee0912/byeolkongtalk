# AGENTS.md 아카이브 — 압축 전 전문 (2026-07-27 스냅샷)

> `AGENTS.md` 가 30,628자까지 불어나 **모든 세션에 자동 로드**되며 ~26k 토큰을 먹던 문제로 2026-07-27 압축.
> 이 파일 = 압축 직전 원본 **전문 바이트 그대로**. 살아있는 규칙(스택·디자인·페르소나·코딩규칙·현행 prod 상태·운영 함정)은 AGENTS.md 에 남았고,
> 여기는 **Phase 0~5 진행 로그 · 세션별 완료 기록 · 역사적 트러블슈팅**의 원장이다.
> ⚠️ **전문 Read 금지 — Grep 으로 필요한 섹션만.**
...
- [ ] **Phase 5** — 사주 도메인 신규 설계
  ...
  - [x] (e2) 결과 / 공유 / 마이페이지 — `lib/saju/closing` 마무리 한마디 추출 + `/api/readings/[id]` 단건 + `/api/readings` 리스트 + `/saju/result` (사주판 + 한마디 카드 + 대화 다시보기 + 공유) + `ShareButtons` (Web Share + 클립보드 + has_sensitive 차단) + `/api/og/saju/[readingId]` next/og 1200×630 + `lib/kakao-share` SDK feed + `/mypage` (프로필 + 잔액 + 히스토리 + 로그아웃/탈퇴) + 랜딩 MY 진입점. ⚠️ **reading [END] → result 는 자동 이동이 아니라 수동 [결과 보기 →] 버튼**(2026-07-26 확인, `tarot/reading/page.tsx:1019`·`saju/reading/page.tsx:641`) — 이 한 탭이 "종료했으나 미열람"의 원인, 픽스는 day 14. `result_viewed_at` 은 "결과를 봤나"가 아니라 "버튼을 눌렀나"를 잰다. `docs/superpowers/specs/2026-07-26-unviewed-results-findings.md`
...
- **카카오 공유**: prod 앱 JS 키 + Web 사이트 도메인(`byeolkongtalk.com`) **등록 완료 = prod 공유 정상** (2026-07-26 사용자 콘솔 확인). ⚠️ **dev 에서만 미리보기가 빈다** — Vercel Deployment Protection(SSO) 때문에 외부 스크래퍼가 OG 이미지/`/cards-webp` 를 못 받는 것이지 코드 결함 아님(dev 에서 보려면 Deployment Protection 해제 = 보안 트레이드오프). ⚠️ 진단 시 주의 2가지: 로그인은 `lib/kakao.ts` 가 `kauth.kakao.com` 으로 **서버 리다이렉트**라 JS SDK 와 무관 → **로그인 정상 ≠ 공유 정상** / `lib/kakao-share.ts` 는 실패 시 `console.warn` + 텍스트 폴백이라 **`error_logs` 에 안 남는다**
