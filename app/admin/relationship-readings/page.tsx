...
async function load(): Promise<{
  ...
  const { data, error } = await supa.rpc("admin_relationship_threads", {
    p_now: new Date().toISOString(),
    p_limit: THREAD_LIMIT,
  });
  // error 를 안 보면 `data ?? []` 가 실패를 "등록된 관계 없음" 으로 위장한다 — 조용히 잘리는 것과
  // 같은 사고다. throw 하지 않고 화면에 드러낸다.
  const failed = !!error;
...
export default async function AdminRelationshipReadings() {
  const { threads, total, truncated, failed } = await load();

  return (
    <div className="space-y-4">
      <h1 className="text-xl font-bold">
        연애 상담 리딩 <span className="text-white/40 text-sm font-normal">전체 {failed ? "—" : total}건 · 스레드(관계) 단위</span>
      </h1>
      ...
      {/* 실패하면 표 자체를 내린다 — 헤더만 남은 빈 표는 "등록된 관계 없음" 과 구분이 안 된다 */}
      {failed ? (
        <LoadFailed block="admin_relationship_threads" />
      ) : (
