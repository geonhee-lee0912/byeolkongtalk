...
// 🔴 getEntitlement 를 daily-report(사주)와 달리 캐시 조회보다도 **앞**에서 부른다 — 이유는 응답
//    모양이 다르기 때문이다. daily-report 의 응답 바디는 entitled 를 안 실어(클라가 calendar
//    응답에서 이미 안다) 자격이 오직 "생성 게이트"로만 쓰이지만, 이 라우트는 캐시 히트·not_drawn·
//    cache_only 응답 **전부**가 entitled 필드를 싣고 cache_only 응답은 gauge 까지 실어야 한다
//    (그래서 프로필·사주 계산도 자격 게이트보다 앞이다). ent 를 맨 앞에서 한 번 구해 모든 이른
//    반환에 재사용하는 이유가 이것이다 — daily-report 모양("게이트를 생성 직전에")으로 "통일"
//    하려 들면 entitled 필드가 빠진 응답이 나가 클라 파싱이 깨진다.
//
