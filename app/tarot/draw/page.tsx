...
export default function TarotDrawPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setSelection(parsed);
      setDeck(shuffleDeck());
      setSlotCards(new Array(count).fill(null));
      setDirections(new Array(count).fill("upright"));
      setMounted(true);
  ...
  const goToReading = () => {
    const drawnCards: DrawnCard[] = slotCards.map((cardId, i) => ({
      position: i,
  ...
  return (
    ...
    >
              ...
              <p className="font-display text-[17px] text-eye-purple leading-tight text-center">
                {allFilled ? (
                  ...
                  <>
                    ...
                    {"  "}
                    <span style={{ color: accent }}>{labels[activeSlot]}</span>
                    {" 자리 카드를 골라줘"}
                  </>
                ) : (
                  "마음이 끌리는 카드를 골라봐"
      ...
      {/* pick 단계: 리딩 방법 선택 + 카드 선택/방향 선택 (동일 너비) */}
      {phase === "pick" &&
        (() => {
          ...
          return (
            <div className={`flex items-center justify-center gap-2 ${stickyClass}`}>
              <button
                onClick={goBack}
      ...
      {/* direction 단계: 고민 상담 시작하기 + 카드 다시 뽑으러 가기 (세로) */}
      {phase === "direction" && (
        <div
            ...
            >
              <button
                ...
                style={{
                  width: CARD_W,
                  height: CARD_H,
                  transform: `scale(${scale})${isCenter ? " translateY(-2px)" : ""}`,
...
function SlotRow({
  ...
  const isRelationship = spreadType === "relationship_5";
          ...
          horizontal ? (
            <Image
              ...
              style={{
                ...
                marginLeft: -SLOT_W / 2,
                marginTop: -SLOT_H / 2,
                transform: `rotate(${direction === "reversed" ? "-90" : "90"}deg)`,
