...
const RUBRIC = [
  ...
  "2. 답 먼저 + 회피구 0회 — 유저의 확답/시점/가부 질문에 응답 앞부분에서 방향을 답했는가. '~못 짚어/알 수 없어/정해줄 수 없어/~하는 일이 아니야' 류 회피 선언이 있으면 위반. **시점 질문('언제쯤 올까'/'연락 올까' 류)의 정답은 조건부 범위 답(시기 범위 + 조건, 예: '~월 즈음, 네가 먼저 연락하면 더 당겨져')이며, '카드가 날짜를 찍어주진 않지만'/'못 박아주긴 어렵지만' 류 헤징·회피 절은 대화 전체에서 단 한 번이라도 등장하면 위반이다(예전엔 대화 전체 1회를 허용했지만 지금은 전면 금지 — 그 절 자체가 회피구다)**. 남은 예외는 의료·법률·재무의 '전문가와 같이' 우회 하나뿐(허용)",
