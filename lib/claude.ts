...
const TURN_CLOSE_GUIDE: Record<TurnClose, string> = {
  ...
  invite:
    "- **턴 마무리 상태: `invite`** — 이번 턴은 물음표 없이 ②(다음 볼거리 예고)로 닫아. **답을 요구하지 않는 게 아니라, 답하고 싶어지게 여는 고리야**: \"약속 날짜 정해지면 어떻게 됐는지 들려줘. 그거 들으면 흐름이 더 또렷해져.\" / \"뭐가 제일 지치게 만들었는지 한 장 더 펼쳐서 볼 수도 있어.\" 물음표만 안 쓰면 돼. (③④ 로 내려도 돼 — ①(질문)로는 못 올려.)",
  settle:
