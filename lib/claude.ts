...
const TURN_CLOSE_GUIDE: Record<TurnClose, string> = {
  ask: "- **턴 마무리 상태: `ask`** — 이번 턴은 §턴 마무리의 ①(구체 질문)으로 닫아. 유저가 방금 준 정보를 먼저 받아 안고(받아 안은 뒤의 질문은 심문이 아니야), 그 정보에 이어지는 구체 질문 **하나**를 얹어. 판과 무관한 막연한 질문은 금지. 정말 더 물을 게 없을 때만 ②③④ 로 내려 — 그건 예외지 기본값이 아니야. ① 보다 위는 없어.",
  invite:
    "- **턴 마무리 상태: `invite`** — 이번 턴은 물음표 없이 ②(다음 볼거리 예고)로 닫아. **답을 요구하지 않는 게 아니라, 답하고 싶어지게 여는 고리야**: \"약속 날짜 정해지면 어떻게 됐는지 들려줘. 그거 들으면 흐름이 더 또렷해져.\" / \"뭐가 제일 지치게 만들었는지, 그 얘기까지 들으면 이 결이 어디서 꼬였는지도 짚어줄 수 있어.\" 물음표만 안 쓰면 돼. 카드를 한 장 더 펼치자거나 사주를 새로 보자는 **제안 자체는 도메인 규칙을 따라** — 여기서 먼저 꺼내지 마. (③④ 로 내려도 돼 — ①(질문)로는 못 올려.)",
  settle:
    "- **턴 마무리 상태: `settle`** — 이번 턴은 ③(소신 정리+여백)이나 ④(공감으로 열어두기)로 닫아. **여기가 제일 아래야 — ①(질문)도 ②(다음 볼거리 예고)도 쓰지 마.** 되묻기는 물론이고 \"궁금하면 그 얘기도 꺼내줄게\" 같은 예고성 고리도 이번 턴엔 열지 마. 유저는 지금 받아주는 마무리가 필요해.",
};
...
// deep(같은 카드 위에서 더 깊이 파는 이어가기)은 짧은 continuationFirstTurnGuide 그대로 유지.
const TAROT_FRESH_CONTINUATION_CONNECTOR = `\n\n## 이어가기 첫 턴 — 카드 새로 펼침\n지난 고민을 이어받아 카드를 새로 뽑았어. 첫 응답을 "지난번에 ~ 얘기 나눴었지" 식으로 지난 맥락을 가볍게 짚으며 열되, 처음 만난 듯 별콩이 소개나 인사는 다시 하지 마.`;
...
export function buildTarotSystemMessage(ctx: TarotReadingContext): {
  ...
  const firstTurnGuide = isFirstTurn
    ? ctx.continuation
      ? ctx.continuation.mode === "fresh"
        ? TAROT_FRESH_CONTINUATION_CONNECTOR + TAROT_FIRST_TURN_GUIDE
        : continuationFirstTurnGuide("카드")
      : TAROT_FIRST_TURN_GUIDE
    : "";
