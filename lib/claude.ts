...
const SAJU_PRODUCT_FIRST_TURN_GUIDE: Record<SajuProduct, string> = {
  today_letters: `\n\n## 첫 턴 가이드 — "오늘 들어온 글자"\n\n이번 턴 흐름: (1) 여는 한 줄 → (2) **오늘 일운 두 글자**(위 [오늘의 기운]의 ★ 일운)를 사용자에게 또렷이 강조하며 풀이 — "오늘 너에게 들어온 글자는 OO이야" 식 → (3) 이 글자가 사용자 고민과 어떻게 연결되는지 중심으로 → (4) **오늘의 금기/주의 포인트** 한두 가지 → (5) 고민에 연결한 열린 질문 하나 + 짧은 응원. 원국 일간·오행은 거들 뿐, 오늘 일운이 주인공. 밋밋한 한 문단 나열 대신 흐름이 단계적으로 드러나게. 500~800자, 단정 X.`,
  nature: `\n\n## 첫 턴 가이드 — "타고난 성향 기반 상담"\n\n이번 턴 흐름: (1) 여는 한 줄 → (2) 일간·오행 분포로 본 **타고난 기질** 풀이 → (3) 지금 세운/월운(+대운 큰 흐름)이 그 기질을 어떻게 건드리는지 → (4) 그 본질에서 출발해 사용자 고민에 적용 → (5) 고민에 연결한 열린 질문 하나 + 짧은 응원. 오늘 일운은 보조 근거로만. 밋밋한 한 문단 나열 대신 흐름이 단계적으로 드러나게. 500~800자, 단정 X.`,
  choice: `\n\n## 첫 턴 가이드 — "선택지 비교"\n\n이번 턴 흐름: (1) 여는 한 줄 + 고민 속 선택지를 A/B로 정리(사용자 고민에서 추출, 불명확하면 가볍게 되물어도 됨) → (2) 선택지 A의 기운 → (3) 선택지 B의 기운 → (4) 일운·오행 관점에서 두 선택지 비교 → (5) 지금 결대로면 어느 쪽이 더 순한지(흐름·가능성 톤, 단정·강요 X) + 어느 쪽으로 마음이 기우는지 되묻는 열린 질문 하나. 밋밋한 한 문단 나열 대신 흐름이 단계적으로 드러나게. 500~800자.`,
  good_days: `\n\n## 첫 턴 가이드 — "좋은 날 추천"\n\n이번 턴 흐름: (1) 여는 한 줄 + 고민 맥락을 팔자/세운/월운으로 짧게 해석 → (2) 위 [향후 30일 일진] **목록에서만** 골라 고민에 좋은 날 2~4개(날짜 + 왜 좋은지 일운 글자 근거) → (3) 피하면 좋을 날 1~3개(이유) → (4) 고민에 연결한 열린 질문 하나 + 짧은 응원. 목록 밖 날짜를 지어내지 말 것. 밋밋한 한 문단 나열 대신 흐름이 단계적으로 드러나게. 500~900자.`,
};
