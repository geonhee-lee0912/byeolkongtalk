...
test("dayType — 계절+유형 결합", () => {
  assert.equal(dayType("신", "오"), "여름의 보석형");
  assert.equal(dayType("무", "인"), "봄의 큰산형");
  assert.equal(dayType("갑", "자"), "겨울의 큰나무형");
  assert.equal(dayType("경", "유"), "가을의 원석형");
});

test("dayType — 미지 폴백", () => {
  assert.equal(dayType("?", "오"), "여름의 별 유형");
...
test("dayType — 실제 calcSaju 출력으로 키 포맷 검증(폴백 아님)", () => {
  ...
  assert.equal(dayType(r.dayStem, r.pillars.month.branch), "여름의 보석형");
});
