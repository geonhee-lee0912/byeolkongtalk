...
const SECTION_GUIDE: Record<FortuneType, string> = {
  ...
  monthly: [
    ...
    `}`,
    ``,
    `[규칙] 모든 문장은 반말 친구 말투. 단정("~할 거야") 금지, 흐름·가능성("~한 흐름이 보여","~해보면 좋아")으로. 각 도메인·주차는 이번 달 월건 기운과 연결해서 구체적으로. 좋기만 한 예언 금지 — 챙길 점도 자연스럽게. **위 JSON 의 모든 필드를 반드시 포함해라 — weekly 는 정확히 4개(week 1·2·3·4), sections 는 정확히 5개(money·work·love·health·study), lucky·timing·balance·intro·note 도 하나도 빠뜨리지 마.** JSON 문자열 안에서 큰따옴표는 escape(\\")하고 줄바꿈은 넣지 마.`,
  ].join("\n"),
  saju_full: [
    `기준 연도: 2026년 (병오년 / 丙午)`,
    ``,
