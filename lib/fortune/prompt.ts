...
const SECTION_GUIDE: Record<FortuneType, string> = {
  ...
  saju_full: [
    ...
    `  "monthly": [`,
    `    { "month": 1,  "body": "<1월 흐름·조언. 6~7문장.>" },`,
    `    { "month": 2,  "body": "<2월. 6~7문장.>" },`,
    `    { "month": 3,  "body": "<3월. 6~7문장.>" },`,
    `    { "month": 4,  "body": "<4월. 6~7문장.>" },`,
    `    { "month": 5,  "body": "<5월. 6~7문장.>" },`,
    `    { "month": 6,  "body": "<6월. 6~7문장.>" },`,
    `    { "month": 7,  "body": "<7월. 6~7문장.>" },`,
    `    { "month": 8,  "body": "<8월. 6~7문장.>" },`,
    `    { "month": 9,  "body": "<9월. 6~7문장.>" },`,
    `    { "month": 10, "body": "<10월. 6~7문장.>" },`,
    `    { "month": 11, "body": "<11월. 6~7문장.>" },`,
    ...
    `}`,
    ``,
    `[규칙] 모든 문장은 반말 친구 말투. 단정("~할 거야") 금지, 흐름·가능성("~한 흐름이 보여","~해보면 좋아")으로. 좋기만 한 예언 금지 — 챙길 점도 자연스럽게. relations2026·mission·halves(first·second)·turning·remedies·year.study·year.moving·year.family 도 빠뜨리지 말고 반드시 포함하고 각 항목은 밀도로 채워(물타기·반복 금지). monthly 는 1~12월 전부, actions 는 정확히 3개. JSON 문자열 안에서 큰따옴표는 escape(\\")하고 줄바꿈은 넣지 마.`,
  ].join("\n"),
  good_days: [
    `오늘 날짜: ${"{{TODAY}}"}`,
    ``,
  ...
  compat: [
    ...
    `}`,
    ``,
    `[규칙] 모든 문장은 반말 친구 말투. 단정("~할 거야") 금지, 흐름·가능성("~한 흐름이 보여","~해보면 좋아")으로. 좋기만 한 예언 금지 — 챙길 점도 자연스럽게. advice 는 정확히 3개. grade 는 위 5개 enum 중 하나로만. summary·chemistry·attraction·conflict·communication·longterm·growth·individual·stages·repair·intimacy 전부 반드시 포함하고 각 항목은 밀도로 채워(물타기·반복 금지). JSON 문자열 안에서 큰따옴표는 escape(\\")하고 줄바꿈은 넣지 마.`,
  ].join("\n"),
  compat_social: [
