...
function genericGuide(opts: {
  ...
  lines.push(
    ...
    `}`,
    ``,
    `[규칙] 모든 문장은 반말 친구 말투(호칭은 '너'). 단정적 예언 금지 — 흐름·가능성("~한 흐름이 보여","~해보면 좋아")으로. 사주 근거(일간·오행·기둥)를 구체적으로 녹이고, 물타기·반복·공허한 미사여구 금지(밀도로 채워라). sections 는 위 heading 순서·개수 그대로 전부 채워라. 따뜻한 마무리.\n[서식] intro 와 각 섹션 body 를 읽기 쉽게 마크다운으로 써라: 핵심 단어·구절을 **굵게**(별표 두 개)로 눈에 들어오게 강조해라 — 문단마다 1~2곳 정도로 너무 아끼지 말 것(단, 한 문단을 통째로 굵게 처리하진 말고 핵심만). 관련된 2~4문장을 한 문단으로 묶고 문단 사이에만 빈 줄(\\n\\n)을 넣어라 — 절대 한 문장마다 띄우지 말 것(한 덩어리는 대략 2~4문단). 나열·목록이 정말 필요할 때만 각 줄을 "- " 로 시작하는 불릿으로. heading 은 맨 앞 이모지 1개 + 제목 형태를 유지해라. 굵게·불릿·문단 나눔 모두 남발 금지 — 가독성을 위한 최소한만. JSON 문자열 안에서 큰따옴표는 escape(\\")하고, 줄바꿈은 반드시 \\n 으로 이스케이프해라(생 줄바꿈 금지).`
  );
  return lines.join("\n");
}
