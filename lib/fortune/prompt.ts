...
const SECTION_GUIDE: Record<FortuneType, string> = {
  ...
  life_full: [
    ...
    `}`,
    ``,
    `[규칙] 모든 문장은 반말 친구 말투(호칭 '너'). 단정적 예언 금지 — 흐름·가능성 화법. 사주·대운 근거를 구체적으로 녹이고 물타기·반복 금지(밀도로 길게). sections 순서 = 타고난그릇 · 인생 큰줄기 · [대운] 각 구간(전부) · 타고난 복 · 3대 고비 · 인생 전환점 · 평생 화두 · 관계/재물/건강 각각 · 말년. 대운 구간은 반드시 [대운] 표 안에서만. 따뜻한 마무리. JSON 문자열 안 큰따옴표는 escape(\\")하고 줄바꿈은 넣지 마.`,
  ].join("\n"),
