...
export const SECTION_GUIDE: Record<FortuneType, string> = {
  daily: [
    ...
    // 목표 대비 +26~+50%(2/2 샘플)로 크게 초과했다. 5문장으로 -1(scripts/p6-2-length-probe.ts 실측).
    ...
    // 🔴 P6-2 Task10(2026-09-20) 실측 조정 — 3문장 지시에도 목표 대비 -21~-30%(2/2 샘플)로 짧게
    // 나왔다. 4문장으로 +1(scripts/p6-2-length-probe.ts 실측). 볼드도 6/6 샘플 전부 0개였다 —
    // "머리말 없이 바로 시작" 지시를 볼드까지 금지하는 뜻으로 읽는 듯해 문구를 보강했다.
  ...
  good_days: [
    ...
    ``,
    `[규칙] 모든 문장은 반말 친구 말투. 단정("~한다","~될 거야") 금지, 흐름·가능성("~한 흐름이 보여","~해보면 좋아")으로. 날짜는 반드시 [향후 30일 일진] 목록 안에서만 고르고, 목록에 없는 날짜·간지를 지어내지 마. 좋기만 한 예언 금지 — 조심할 날도 자연스럽게.`,
    `[서식] 섹션은 \`## 제목\` 헤딩으로 나누고, 각 섹션 본문은 읽기 쉽게 마크다운으로: 핵심 단어·구절을 문단마다 1~2곳 **굵게**(한 문단 통째 굵게 금지), 관련된 2~4문장을 한 문단으로 묶고 문단 사이에만 빈 줄로 나눠라. 나열형(좋은 날·조심할 날·실천 등)은 각 줄 "- " 불릿으로 정리하면 좋고, 가장 중요한 팁 1개는 "> " 로 시작하는 콜아웃 줄로 뽑아도 돼(남발 금지). 코드펜스는 쓰지 마.`,
  ].join("\n"),
  ...
  life_full: [
    ...
    `  "daeunLines": [`,
    `    "<★위 [대운] 표의 각 구간마다 {startAge, line} 객체 하나씩. startAge 는 그 구간 시작 나이 정수(표의 'N0~N9세'의 앞 숫자 그대로, 예: 32). line 은 '이 사람의 사주(일간·오행)와 이 대운이 만나서' 그 10년에 도드라지는 일을 한 줄로 — 25~45자, 정확히 1문장, 단정 말고 흐름·가능성 화법('~흐름이 보여','~하기 좋아'). 뻔한 일반론 말고 이 사주에 특정된 구체적 장면으로. [대운] 표 구간 전부(대개 8~9개)를 순서대로, startAge 는 표에 있는 값만.>"`,
    `  ],`,
    `  "sections": [`,
  ...
  saju_report_card: [
    ...
    `}`,
    ``,
    `[규칙] 반말 친구 말투. 학점은 사주 근거로 정직하게(전부 A 금지, 낮아도 겁주지 말고 재치있게). scores 는 위 5개 도메인 그대로. 단정예언 금지·흐름화법. 따뜻한 마무리. [서식] 서술형 필드(도입·코멘트·총평·한마디)는 읽기 쉽게 마크다운으로: 핵심 단어·구절을 문단마다 1~2곳 **굵게**(핵심만, 한 문단 통째 굵게 금지), 관련된 2~4문장을 한 문단으로 묶고 문단 사이에 빈 줄로 나눠라(한 문장마다 띄우지 말 것). 나열형은 각 줄 "- "로 시작하는 불릿(어울리면 3~5개), 가장 중요한 팁 1개는 "> "로 시작하는 콜아웃(항목당 최대 1개, 남발 금지). 억지로 다 넣진 말 것. JSON 문자열 안 큰따옴표는 escape(\\").`,
  ].join("\n"),
  life_graph: [
    ...
    `}`,
    ``,
    `[규칙] 반말 친구 말투. score 는 대운·사주 근거로 기복 있게(전부 90+ 금지). decades 는 [대운] 표 안에서만. 단정예언 금지·흐름화법. 저점도 겁주지 말고 지혜로. [서식] 서술형 필드(도입·구간 body·최고/저점·한마디)는 읽기 쉽게 마크다운으로: 핵심 단어·구절을 문단마다 1~2곳 **굵게**(핵심만, 한 문단 통째 굵게 금지), 관련된 2~4문장을 한 문단으로 묶고 문단 사이에 빈 줄로 나눠라. 나열형은 각 줄 "- "로 시작하는 불릿, 가장 중요한 팁 1개는 "> "로 시작하는 콜아웃(항목당 최대 1개, 남발 금지). 억지로 다 넣진 말 것. headline 은 짧은 한 줄이라 서식 없이. JSON 문자열 안 큰따옴표는 escape(\\").`,
  ].join("\n"),
};
