...
export function buildPairNarrativeSystem(
  ...
  return [
    ...
    "이 순서로 자연스럽게 이어서 써(각 부분을 라벨로 표시하지 마): 먼저 오늘 일진이 둘 사이를 어떻게 건드리는지로 열고, 이어서 위에 적힌 오늘 신호가 실제 하루에서 어떤 장면으로 나타날지 구체적으로 그리고, 그다음 너희 고정된 결(십신 관계·연월조화)이 오늘과 겹쳐 어디를 편하게 하고 어디를 걸리게 하는지 짚고, 마지막은 따뜻한 한 줄로 맺어.",
    "규칙: 반말, 단정적 예언 금지(흐름·가능성·선택). 한쪽을 탓하지 말고 둘의 흐름으로 말할 것. 같은 말을 바꿔 쓰며 늘리지 말고(분량은 장면과 예시로 채운다), 위에 준 신호 낱말을 라벨처럼 앞세우지 말 것. '마무리로'·'정리하면'·'오늘의 신호를 보면'처럼 글의 구조를 설명하는 말로 문단을 열지 말고, 바로 그 내용으로 들어갈 것." +
      (goodList
...
export function buildCardReportSystem(i: CardReportPromptInput): string {
  ...
  return [
    ...
    gaugeLine(i.gauge),
    "",
    "[역할] 화면엔 이미 카드 그림·이름·정역·키워드가 떠 있다(지금 이 리포트를 읽는 사람 기준). 아래는 같은 카드를 만나는 무료(비구독) 독자가 읽는 소개 글이다 — 너는 이미 결제한 사람에게 쓰는 거라 이 소개를 반복하거나 다시 읽히면 안 된다:",
    i.freeTaste ? `«${i.freeTaste}»` : "«(무료 소개 없음 — 그래도 카드가 어떤 카드인지는 화면 키워드가 말한다)»",
    "그러니 위 소개와 같은 메시지를 되풀이하지 말고, 카드 상징이나 전통 의미를 처음 소개하듯 재설명하지 마. 다만 place 블록에서 이 카드가 사주 위 어디에 떨어지는지 짚을 때 그 카드 성격을 한 호흡에 자연스럽게 실어 말하는 것은 된다 — 재소개가 아니라 자리를 짚는 문장 안에 녹이는 것이면 괜찮다(무료 소개도 상징 재설명도 막히면 유료 독자는 이 카드가 어떤 카드인지 키워드 말고는 들을 데가 없다). 이 리포트가 할 일은 **이 카드가 네 사주 일간·오늘 일진의 어디에 떨어지는지** — 어디서 받쳐주고 어디서 부딪히는지 — 와, 그래서 오늘 벌어질 장면과 그때의 선택이다.",
    "",
